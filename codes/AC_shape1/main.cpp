...
void initial()
{
    ...
    for(int j=N+1+5;j<=N+2*L0+1-5;j++)
    {
        Car[8][j].dao=true;
        Car[9][j].dao=true;
    }
    for(int j=N+1+10;j<=N+2*L0+1-10;j++)
    {
        Car[10][j].dao=true;
        Car[11][j].dao=true;
    }
    for(int j=N+1+15;j<=N+2*L0+1-15;j++)
    {
        Car[12][j].dao=true;
        Car[13][j].dao=true;
    }
...
void add_minus()
{
    ...
    for(int i=1;i<=B;i++)
    {
        for(int j=2*N+2*L0+1;j>=1;j--)
        {
            ...
            Car[i][j].v=Min(Car[i][j].v,dn);//减速
            //变道&更新位置
            if(dn<Min(Car[i][j].v+1,Vmax)&&d_right>dn&&d_left<=d_right&&Car[i+1][j].occupy==false&&Car[i+1][j].dao==true)
            {
                Car[i][j].occupy=false;
            ...
            else if(dn<Min(Car[i][j].v+1,Vmax)&&d_left>dn&&d_left>=d_right&&Car[i-1][j].occupy==false&&Car[i-1][j].dao==true)
            {
                Car[i][j].occupy=false;
                Car[i-1][j].occupy=true;
                Car[i-1][j].v=Car[i][j].v;
